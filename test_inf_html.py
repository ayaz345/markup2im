...
class TrainingConfig:
    image_size = (64, 64)  # the generated image resolution
    ...
    output_dir = 'images_rendered_html'  # the model namy locally and on the HF Hub
...
config.dataset_name1 = "yuntian-deng/im2html-100k"
dataset1 = load_dataset(config.dataset_name1, split="train")
dataset = dataset1
dataset = dataset.add_column("formula", dataset['html'])
dataset = dataset.shuffle(seed=42)
...
state_dict = torch.load('model_latest_html', map_location='cpu')
state_dict_out = {}
for k in state_dict:
